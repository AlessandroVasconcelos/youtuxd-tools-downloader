...
def extrair_capa(info_dict):
    ...
    # Se não achar direto, procura na lista de várias resoluções
    if not url_capa and info_dict.get("thumbnails"):
        url_capa = info_dict["thumbnails"][-1].get("url")
        
    # Se for playlist, caça a capa do primeiro vídeo válido
    if not url_capa and 'entries' in info_dict and info_dict['entries']:
        for video in info_dict['entries']:
            if isinstance(video, dict):
                ...
                if url_capa:
                    break # Achou uma capa, para de procurar
                    
    # Fallback: Imagem padrão com as cores do YouTube (Branco/Cinza e Vermelho)
    if not url_capa:
...
# --- ETAPA 1: BACKEND RÁPIDO (SÓ METADADOS) ---
...
# --- ETAPA 2: BACKEND DE DOWNLOAD (BAIXA O ARQUIVO REAL) ---
...
# --- INTERFACE ---
# Inicializa a memória (session_state)
...
# Cabeçalho Principal
st.markdown("<h1>🎥 SkyDown Pro</h1>", unsafe_allow_html=True)
...
if st.session_state.info_video:
    ...
    # Chama a função infalível para pegar a capa
    ...
    # Monta todo o HTML de uma vez para o Streamlit não bugar
    card_html = f"""
    ...
    if st.session_state.fazer_scroll:
        # Usamos time.time() para criar um script "inédito" toda vez e driblar o cache do Streamlit
        js_scroll = f"""
        <script>
        // Timestamp: {time.time()}
        setTimeout(function() {{
            try {{
                ...
                if (cards.length > 0) {{
                    // Pega o último card renderizado e rola até o TOPO dele
                    cards[cards.length - 1].scrollIntoView({{behavior: 'smooth', block: 'start'}});
                }}
            }} catch(e) {{
                console.log("Erro no scroll automático:", e);
            }}
        }}, 600);
        </script>
        """
...
# Rodapé Customizado
st.markdown("<div class='custom-footer'>SkyDown Pro © 2026 - Alessandro Vasconcelos</div>", unsafe_allow_html=True)